...
parser.add_argument('--input_image', default='./images/profile_02.jpg', type=str, help='test image')
parser.add_argument('--style_image', default='./images/style.jpg', type=str, help='style image')
parser.add_argument('--weight', type=str, default='./model/decoder.pth', help='decoder weight file')
...
if __name__ == '__main__':
	...
	with torch.no_grad():
		vgg_model = torch.load('./model/vgg_normalized.pth')
		...
		if torch.cuda.is_available():
			...
			style_tensor = transforms.ToTensor()(style_image).unsqueeze(0).cuda()
			net.cuda()

		out_tensor = net([input_tensor, style_tensor], alpha = opt.alpha)

		save_image(out_tensor, out_dir + 'output.jpg')